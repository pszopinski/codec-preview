...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow) {
    ...
    for (auto codecWidget : codecWidgets) {
        ...
        connect(codecWidget, &CodecParametersWidget::parametersChanged, this, &MainWindow::settingsChanged);
    }

    // stream on settings changed
    connect(this, &MainWindow::settingsChanged, this, &MainWindow::stream);

    // react on stats changed
    connect(ui->videoPlayback, &VideoPlaybackWidget::statsChanged, ui->videoInfo,
            &VideoStatisticsWidget::onStatsChange);

    // stream on tab changed
    connect(ui->codecTabsWidget, &QTabWidget::currentChanged, this, &MainWindow::settingsChanged);
...
void MainWindow::on_actionOpen_from_camera_triggered() { openFromCamera(); }
...
void MainWindow::on_actionChange_raw_host_triggered() {
    bool ok;
    QString input;
    input = QInputDialog::getText(this, tr("Change host"), tr("Multicast host for streaming (raw):"), QLineEdit::Normal,
                                  rawVideoHost, &ok);
    if (ok) {
        rawVideoHost = input;
        resetPlayback();
...
void MainWindow::on_actionChange_raw_port_triggered() {
    bool ok;
    QString input;
    input = QInputDialog::getText(this, tr("Change port"), tr("Multicast port for streaming (raw):"), QLineEdit::Normal,
                                  rawVideoPort, &ok);
    if (ok) {
        rawVideoPort = input;
        resetPlayback();
...
void MainWindow::on_actionChange_encoded_host_triggered() {
    bool ok;
    QString input;
    input = QInputDialog::getText(this, tr("Change host"), tr("Multicast host for streaming (encoded):"),
                                  QLineEdit::Normal, encodedVideoHost, &ok);
    if (ok) {
...
void MainWindow::on_actionChange_encoded_port_triggered() {
    bool ok;
    QString input;
    input = QInputDialog::getText(this, tr("Change port"), tr("Multicast port for streaming (encoded):"),
                                  QLineEdit::Normal, encodedVideoPort, &ok);
    if (ok) {
...
void MainWindow::on_actionMacroblocks_triggered() { getSingleFrame(); }
...
QString MainWindow::getStreamingParameters() {
    ...
    CodecParametersWidget *codecWidget = codecWidgets.at(ui->codecTabsWidget->currentIndex());
...
void MainWindow::parseCameraNameProbeOutput(int a, QProcess::ExitStatus b) {
    // silence warning
    (void)a;
    (void)b;
    ...
    QString inputLocation = QString("video=") + QString(match.captured().toUtf8().constData());
    // settingsChanged();
...
void MainWindow::on_actionOriginal_size_toggled(bool arg1) {
    if (arg1)
        ui->videoPlayback->originalSize(ui->videoInfo->getFrameSize());
    else
        ui->videoPlayback->restoreDefaultSize();
}
