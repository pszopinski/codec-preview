...
inline bool oMHashEqualToKey(const Key &key1, const Key &key2) {
...
inline bool oMHashEqualToKey(const Ptr *key1, const Ptr *key2) {
  ...
  public:
    ...
    class iterator {
      ...
      public:
        ...
        const Key &key() const { return *qllIter; }
        ...
        iterator operator-(int i) const { return operator+(-i); }
    ...
    class const_iterator {
      ...
      public:
        ...
        const Key &key() const { return *qllConstIter; }
        ...
        const_iterator operator-(int i) const { return operator+(-i); }
    ...
    class OMHash : public QHash<Key, OMHashValue> {
      public:
        bool operator==(const OMHash &other) const {
            ...
            typename QHash<Key, OMHashValue>::const_iterator it1 =
                this->constBegin();
            typename QHash<Key, OMHashValue>::const_iterator it2 =
                other.constBegin();
            ...
            while (it1 != this->end()) {
                ...
                if ((v1.first != v2.first) ||
                    !oMHashEqualToKey<Key>(it1.key(), it2.key())) {
                    return false;
                }
                ++it1;
                ++it2;
...
template <typename Key, typename Value> void OrderedMap<Key, Value>::clear() {
    data.clear();
    insertOrder.clear();
}
...
template <typename Key, typename Value>
Value OrderedMap<Key, Value>::value(const Key &key,
...
operator=(const OrderedMap<Key, Value> &other) {
    copy(other);
    return *this;
}

template <typename Key, typename Value>
bool OrderedMap<Key, Value>::
operator==(const OrderedMap<Key, Value> &other) const {
...
template <typename Key, typename Value>
typename OrderedMap<Key, Value>::const_iterator
OrderedMap<Key, Value>::begin() const {
...
template <typename Key, typename Value>
typename OrderedMap<Key, Value>::const_iterator
OrderedMap<Key, Value>::end() const {
    return const_iterator(insertOrder.end(), &data);
}

template <typename Key, typename Value>
typename OrderedMap<Key, Value>::iterator
OrderedMap<Key, Value>::erase(iterator pos) {
...
template <typename Key, typename Value>
typename OrderedMap<Key, Value>::iterator
OrderedMap<Key, Value>::find(const Key &key) {
...
template <typename Key, typename Value>
typename OrderedMap<Key, Value>::const_iterator
OrderedMap<Key, Value>::find(const Key &key) const {
